...
namespace BadaoShen
{
    internal static class BadaoMath
    {
        ...
        public static Vector2? GetLastWallPoint(Vector2 from, Vector2 to, float step = 25)
        {
            ...
            if (Fstwall != null)
            {
                ...
                for (var d = step; d < firstwall.Distance(to) + 1000; d = d + step)
                {
        ...
        public static bool InMiddileWall(Vector2 firstwall, Vector2 lastwall)
        {
            ...
            for (var i = 0; i <= 350; i = i + 10)
            {
                ...
                var flags = NavMesh.GetCollisionFlags(testpoint.X, testpoint.Y);
                if (!flags.HasFlag(CollisionFlags.Wall) && !flags.HasFlag(CollisionFlags.Building)) return false;
            }
