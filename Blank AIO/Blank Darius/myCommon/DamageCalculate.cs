...
namespace myCommon
{
    public static class DamageCalculate
    {
        ...
        {
            if (target == null || target.IsDead || target.IsZombie) return 0;
            ...
            if (target.CharacterData.CharacterName == "Mordekaiser") damage -= target.Mana;
        ...
        public static float GetPassiveCount(AIBaseClient target)
        {
        ...
        public static float GetRDamage(AIBaseClient target)
        {
            ...
            // passive
            if (target.HasBuff("DariusHemo")) Damage += Damage * GetPassiveCount(target) * 0.2f;

            if (target.CharacterData.CharacterName == "Moredkaiser") Damage -= target.Mana;
            ...
            // kindred r
            if (target.HasBuff("KindredRNoDeathBuff")) Damage = 0;

            // tryndamere r
            if (target.HasBuff("UndyingRage") && target.GetBuff("UndyingRage").EndTime - Game.Time > 0.3) Damage = 0;

            // kayle r
            if (target.HasBuff("JudicatorIntervention")) Damage = 0;

            // zilean r
            if (target.HasBuff("ChronoShift") && target.GetBuff("ChronoShift").EndTime - Game.Time > 0.3) Damage = 0;

            // fiora w
            if (target.HasBuff("FioraW")) Damage = 0;
